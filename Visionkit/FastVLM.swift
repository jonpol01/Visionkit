...
// Opt out of default MainActor isolation for this file —
// MLX Module subclasses must be nonisolated.
    ...
    fileprivate class VisionModelCoreML {
        ...
        func load() throws -> MLModel {
            try lock.withLock {
                ...
                if let url = bundle.url(forResource: "fastvithd", withExtension: "mlmodelc") {
                    let m = try MLModel(contentsOf: url)
                    _model = m
                    return m
                }

                // Try bundle's model/ subfolder
                var packageURL: URL
                if let resourceURL = bundle.resourceURL {
                    let modelDir = resourceURL.appendingPathComponent("model")
                    packageURL = modelDir.appendingPathComponent("fastvithd.mlpackage")
                } else {
        ...
        public func encode(_ image: MLXArray) -> MLXArray {
            ...
            return data.withUnsafeMutableBytes { (ptr: UnsafeMutableRawBufferPointer) in
                ...
                let imageFeatures = output.featureValue(for: "image_features")!.multiArrayValue!
                precondition(imageFeatures.shape == [1, 256, 3072] as [NSNumber])
...
public class FastVLMProcessor: UserInputProcessor {
    ...
    public func prepare(prompt: UserInput.Prompt, imageTHW: THW?) -> String {
        ...
        case .messages(let msgs):
            messages = msgs.compactMap { dict in
                guard let role = dict["role"] as? String,
                      let content = dict["content"] as? String else { return nil }
                return ["role": role, "content": content]
            }
        case .chat(let chatMessages):
            messages = chatMessages.map { ["role": "user", "content": $0.content] }
        }
...
public class FastVLM: Module, VLMModel, KVCacheDimensionProvider {
    ...
    static public var modelConfiguration: ModelConfiguration {
        // Look for config.json in the bundle's model/ subfolder (folder reference)
        let bundle = Bundle(for: FastVLM.self)

        // Folder reference: files are at <bundle>/model/config.json
        if let resourceURL = bundle.resourceURL {
            let modelDir = resourceURL.appendingPathComponent("model")
        ...
        // Flat bundle: config.json is directly in bundle resources
        if let url = bundle.url(forResource: "config", withExtension: "json") {
            let dir = url.resolvingSymlinksInPath().deletingLastPathComponent()
            return ModelConfiguration(directory: dir)
        }

        // Last resort: source tree (only works on macOS)
        let sourceDir = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .appendingPathComponent("model")
            .resolvingSymlinksInPath()
        return ModelConfiguration(directory: sourceDir)
    }
